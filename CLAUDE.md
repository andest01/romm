...
| `playwright-cli`         | Driving a browser cheaply to look at or check the running app. It's a `frontend/` devDependency: run it from `frontend/` as `npx playwright-cli`; never install it globally. To write or fix e2e specs, use the `playwright-test-*` agents instead (see `frontend/e2e/README.md`). |
