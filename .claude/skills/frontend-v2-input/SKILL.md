...
Verification adds a breakpoint sweep; see `review-polish`. In Storybook, use the viewport toolbar presets (RomM xs/sm/md tiers, Steam Deck for lg, AYN Thor top/bottom screens). They're defined once in `src/v2/devices.ts`, which also drives the Playwright device projects (tag an e2e test `@devices` to run it on each, with a virtual gamepad on the handhelds). A story that needs a tier sets `globals: { viewport: { value: "rommPhoneXs" } }` instead of writing `data-bp` itself. Layout that depends on `html[data-bp~="xs"]` needs a canvas **under 600px** wide (e.g. **390×844 · RomM phone**), not just `useBreakpoint()` switching subtabs at tablet widths.
