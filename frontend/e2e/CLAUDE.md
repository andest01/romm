...
- **Target devices live in `src/v2/devices.ts`,** the single home for both Storybook's viewports and the Playwright device projects. Add a device there, never in the config or a spec. A test joins the device sweep with `{ tag: "@devices" }`; keep that for tests whose behaviour depends on size, touch or gamepad input, since each tagged test runs six more times. Handhelds get a virtual gamepad through the `gamepad` option.
- **Pick the right browser tool.** `playwright-cli` (the skill; run `npx playwright-cli` from `frontend/`, never install it globally) is for looking at or checking the app, cheaply. The `playwright-test-*` agents are for writing and healing specs, because they run through the suite. Don't hand-write a spec by driving `playwright-cli` and pasting its output.
- **Playwright's test agents** (`playwright-test-planner`, `-generator`, `-healer` in `.claude/agents/`, MCP server `playwright-test` in `.mcp.json`):
  - **Plans:** they live in `e2e/specs/`, and every plan names `e2e/seed.spec.ts` as its seed. The seed runs through the suite's fixtures, signed in, so agents start where tests start.
  - **Generated specs follow every rule in this file:** `test`/`expect` from `./fixtures/test`, `test.use({ storageState })` for identity, `{ tag: "@devices" }` only when size or input matters, real assertions, and role or label selectors. Fix a generated spec that doesn't, before committing it.
  - **Never hand-edit the agent files;** they're generated. After a Playwright upgrade, regenerate them from the repo root with `node frontend/node_modules/playwright/cli.js init-agents --loop=claude -c frontend/playwright.config.ts`. Then restore `.mcp.json` (the generated one is tied to one OS and to a root `npx`) and move any new root `specs/` back into `e2e/specs/`.
